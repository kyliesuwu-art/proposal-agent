...
class TaskControlService:
    ...
    def receive(self, message) -> ControlResult | None:
        ...
        if command_name == "help":
            return ControlResult("当前支持：发送方案需求、确认、上传 Markdown、生成Word、重新生成Word、状态、重发Word。")
        if command_name == "status":
            return ControlResult(self._status(message, _STATUS_RE.fullmatch(text).group(1)))
        return self._resend(message, _RESEND_RE.fullmatch(text).group(1))
    ...
    def _resend(self, message, task_id: str | None) -> ControlResult:
        task = self._owned_task(message.userid, message.chatid, task_id)
        if task is None:
        ...
        if successful:
            path = self._safe_word_file(task, successful[-1])
            if path:
                return ControlResult(f"正在重新发送已生成的 Word 文件。\n任务编号：{task.task_id}", path)
            return ControlResult("已找到成功记录，但文件当前不可用。请发送“重新生成Word”创建新的生成任务。")
        if not jobs:
            return ControlResult("当前没有已生成的 Word 文件。请发送“生成Word”。")
        latest = jobs[-1]
        if latest.status in {ArtifactJobStatus.QUEUED, ArtifactJobStatus.RUNNING}:
            return ControlResult("Word 文档正在生成，请等待完成。")
        return ControlResult("最近一次 Word 生成失败。请发送“重新生成Word”创建新的生成任务。")
