...
- 输出格式：Markdown
- **企微长连接**：`wecom-aibot-python-sdk`（`wecom-aibot-sdk-python>=0.1.7`），WebSocket 长连接收发消息，入口脚本 `src/wecom_bot.py`

## 项目结构
Core/
  AGENTS.md         # 本文件，架构说明与编码约定
  ...
  src/
    ...
    main.py
    wecom_bot.py        # 企微长连接 WebSocket 入口（收发消息验证脚本）
...
## 企微长连接接入
- **SDK**：`wecom-aibot-sdk-python>=0.1.7`（PyPI 包名，import 名为 `wecom_aibot_sdk`）
- **入口**：`uv run python src/wecom_bot.py`（独立脚本，尚未接入业务管线）
- **连接方式**：WebSocket 长连接，`WSClientOptions(bot_id, secret)` → `WSClient(opts)` → `client.connect_async()`
- **消息回调**：`client.on("message", handler)`，handler 是 `async def on_message(frame) -> None`
- **回复消息**：必须用 `await client.reply_stream(frame, stream_id, content, finish=True)`，不能用 `reply()` 发纯 text（会报 40008 错误）
- **消息体结构**（实际收到的 JSON）：
  ```json
  {
    "msgid": "93e4f27c...",
  ...
  ```
- **`.env` 变量**：`AIBOT_BOT_ID`（Bot ID）、`AIBOT_SECRET`（Secret）
- **用户标识**：`body["from"]["userid"]` 是用户唯一标识（当前为拼音工号格式，如 `WuSu`），可用于多轮对话状态管理
