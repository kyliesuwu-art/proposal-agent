...
### D-017 正式 Word/PPT 成品与审阅稿分层

- 状态：有效
- 日期：2026-09-11
- 决策：`proposal.md` 与来源侧车继续是事实、引用及待确认信息的唯一审阅源。正式 Word/PPT 使用只读派生文本和 slide plan：移除可见来源、图片来源、内部 ID、草稿提示和方括号状态标签；未知项目参数不以字符串替换伪装为事实，而集中为“深化设计输入条件”。正式 PPT 的 briefing 交付范围为 15～25 页，默认目标约 18～20 页，来源 ID 保留在内部 plan/sidecar，不显示在成品页面。
- 影响：每个 Markdown 图片块均会被 Word 解析器消费；被拒绝图片直接跳过，绝不能作为 Markdown 文本落入 DOCX。图片决策可按 Word/PPT 分别选择。业务员目录只保留 DOCX 与 PPTX，plan、sidecar、审计和报告置于内部目录。
- 验收：结构、文字泄漏、OOXML media 和 PPT 几何审计可离线验证；它们不替代真实逐页视觉审阅。没有页面渲染器时 DOCX/PPTX 视觉状态必须为 BLOCKED，不能标记 PASS。
