...
- `pipeline.query()` 返回渠道无关的 `QueryResult`；CLI 仅负责将其渲染为 Markdown，
  供后续企微、Word、PPT、PDF 等交付方式复用，不改变既有检索策略。
- 引用统一采用 `[来源: <source_file>, 第 <page_number> 页]`。文件名与页码共同构成
  引用身份；旧 Chroma `slide_number` 只在输出层映射为 `page_number`，不迁移数据库。
- 引用校验只接受本次主命中或相邻页中的“文件名 + 页码”组合。无效引用保留在生成正文
  以避免静默改写模型输出，同时写入 `warnings`，由调用方决定是否要求人工复核。

## 检索 V1：软类型信号与按需上下文（2026-08）

- 既有 Chroma 向量召回仍是第一阶段，`doc_type=proposal` 的默认基础过滤不变；历史
  `proposal_type` 是逐页 LLM 自由标签，故不再是默认硬过滤条件。
- 只有用户原问题明确包含小型词表中的类型词时，才尝试 Chroma 的精确类型过滤；候选不足
  时自动回退至不按类型过滤的候选集。类型映射仅覆盖当前已知同义词，未知值不强行归类。
- 每次查询从 Chroma 取得约 `top_n × 3` 候选，纯 Python rerank 以向量距离为主，关键词
  重合与 exact/related 类型匹配仅作小幅加分；分数、原因和回退信息进入 `QueryResult`。
- 主命中始终保留。相邻页仅在同文件、主页面内容过短或存在关键词关联时作为 `supporting`
  页加入，并受每文件 4 页、总计 14 页、约 16000 字符预算限制。
- 父子块与摘要索引需要重建历史索引，故延期；本阶段只改善现有 Chroma 数据的查询编排。

## 检索 V2：隔离测试库与向量 + 词法融合（2026-08）

- `keywords`、`entities`、`parameters` 是页面检索字段，不等同于 Chroma metadata：它们是
  可重复、可解释、可搜索的多值文本，连同完整页面 payload 保存在独立 SQLite 词法索引中；
  Chroma metadata 仅保留 document/version/source/page 等标量身份与过滤字段，避免把复杂内容
  序列化进 metadata。
- V2 同时保留 Chroma 语义召回和本地 SQLite FTS5 精确词法召回。对于 PCS、型号、35kV、
  100MW/200MWh 等问题，词法索引提供可解释的精确命中；用稳定的 RRF(k=60) 融合而非硬过滤，
  使自然语言语义结果和精确参数结果都能进入候选。proposal_type 仍仅是软信号，不作为硬过滤。
- 本批关键词/实体抽取不调用 LLM、DashScope 或其他外部 API。规则只覆盖标题/章节、受维护的
  领域同义词、缩写、电压和功率/能量参数，以及保守的型号/项目名；未知词保留原文。
